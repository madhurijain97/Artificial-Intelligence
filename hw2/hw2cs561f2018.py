...
class Node:
    def __init__(self, splaVal, lahsaVal, backtrackSplaVal, backtrackLahsaVal, id):
...
def minimaxSpla(startIndex, splaDaysList, lahsaDaysList, path):
        ...
        for details in newSplaList[startIndex:]:
            ...
            if compatibility(splaDaysList, details[5:], spacesParking) == True:
                ...
                #print("path=", path, "splaVal=", node.splaVal, "lahsaval=", node.lahsaVal, "backtrack spla=", node.backtrackSplaVal, "backtrack lahsa=", node.backtrackLahsaVal)
...
with fileOpen as inputFile:
    ...
    for details in allApplicantList:
        id = details[:5]
        if id in lahsaApplicantList or id in splaApplicantList:
            continue
    ...
    """print("All applicant list", allApplicantList)
    print("SPLA satisfied list", splaSatisfiedList)
    print("Lahsa Satisfoed list", lahsaSatisfiedList)
    print("spla pick up list", splaPickUpList)
    print("SPla lahsa satisfied list", lahsaSplaSatisfiedList)
    print("new SPla list", newSplaList)
    print("new lahsa list", newLahsaList)"""
...
for details in newSplaList:
    ...
    #print(additionSplaList, details[:5], "splainitialfor")
    ...
    node.backtrackSplaVal, node.backtrackLahsaVal = comparison(node.backtrackSplaVal, backtrackSpla, node.backtrackLahsaVal, backtrackLahsa, "spla")
    #finalSplaVal, finalLahsaVal = comparison(node.backtrackSplaVal, finalSplaVal, node.backtrackLahsaVal, finalLahsaVal, "spla")
    if node.backtrackSplaVal > finalSplaVal:
        finalSplaVal = node.backtrackSplaVal
        finalLahsaVal = node.backtrackLahsaVal
        finalId = details[:5]
    #print(finalSplaVal, finalLahsaVal)
    path.pop()
    #print("path=", path, "splaVal=", node.splaVal, "lahsaval=", node.lahsaVal, "backtrack spla=", node.backtrackSplaVal, "backtrack lahsa=", node.backtrackLahsaVal)

#print(finalSplaVal, finalLahsaVal, finalId)
